...
<<<<<<< HEAD
    on April 05, 2022, at 12:34
=======
    on April 05, 2022, at 18:54
>>>>>>> 4c582c7279a603fc4fc7ef8eb02f481ddb1a3f61
...
# Initialize components for Routine "operator_check"
operator_checkClock = core.Clock()
text_2 = visual.TextStim(win=win, name='text_2',
    text='Waiting for the operator...',
    font='Open Sans',
    pos=(0, 0), height=0.04, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
# ------Prepare to start Routine "operator_check"-------
continueRoutine = True
# update component parameters for each repeat
key_resp.keys = []
key_resp.rt = []
_key_resp_allKeys = []
# keep track of which components have finished
operator_checkComponents = [text_2, key_resp]
for thisComponent in operator_checkComponents:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
operator_checkClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "operator_check"-------
while continueRoutine:
    # get current time
    t = operator_checkClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=operator_checkClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
...
thisExp.nextEntry()
# the Routine "operator_check" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
